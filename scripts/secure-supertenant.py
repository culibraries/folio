...
def main():
    ...
    print(
        "Successfully created user {} with id {}".format(
            args.user_name, newuser_json["id"]
        )
    )
    ...
    payload = json.dumps(
        {"username": args.user_name, "password": args.password}
    ).encode("UTF-8")
...
def parse_command_line_args():
    ...
    parser.add_argument(
        "-u", "--user-name", help="okapi super user username", required=True
    )
    parser.add_argument(
        "-p", "--password", help="okapi super user password", required=True
    )
    parser.add_argument(
        "-o",
        "--okapi-url",
        help="Default http://localhost:9130",
...
def okapi_get(url, tenant=None):
    ...
    try:
        ...
        sys.exit(" - ".join(["ERROR", "GET", e.url, str(e.status), str(e.read())]))
...
def okapi_post(url, payload, tenant=None, return_headers=False):
    ...
    try:
        ...
        sys.exit(
            " - ".join(
                ["ERROR", "POST", e.url, str(e.status), str(e.read().decode("utf-8"))]
            )
        )
    return response_data
...
def enable_module(module_id, okapi_url, tenant=None):
    tenant = tenant or "supertenant"
    payload = json.dumps({"id": module_id}).encode("UTF-8")
    return okapi_post(okapi_url + "/_/proxy/tenants/{}/modules".format(tenant), payload)


def create_user_mod_users(username, okapi_url, id=None, tenant=None):
    id = id or str(uuid.uuid4())
    payload = json.dumps({"id": id, "username": username, "active": "true"}).encode(
        "UTF-8"
    )
...
def add_permissions(id, permissions, okapi_url):
    payload = json.dumps({"userId": id, "permissions": permissions}).encode("UTF-8")
